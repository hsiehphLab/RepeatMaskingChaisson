...
comask: CombineMask.cpp
	g++ -O2  $< -o $@  -I $(CONDA_PREFIX)/include -L$(CONDA_PREFIX)/lib -lhts -Wl,-rpath,$(CONDA_PREFIX)/lib  -lhts -lz -lpthread

bemask: MaskBed.cpp
	g++ -O2  $< -o $@  -I $(CONDA_PREFIX)/include -L$(CONDA_PREFIX)/lib -lhts -Wl,-rpath,$(CONDA_PREFIX)/htslib/lib  -lhts -lz -lpthread

mask2bed: MaskedToBed.cpp 
	g++ -O2  $< -o $@  -I $(CONDA_PREFIX)/include -L$(CONDA_PREFIX)/lib -lhts -Wl,-rpath,$(PWD)/htslib/lib  -lhts -lz -lpthread
